...
function enhancedUpdateTemplatePreview() {
  ...
  // Preview alanının gerçek boyutlarını al
  const previewRect = preview.getBoundingClientRect();
  const maxWidth = previewRect.width - 32; // Padding için margin
  const maxHeight = previewRect.height - 32;
  
  // mm to px dönüşümü (1mm = ~3.78px)
  const widthPx = width * 3.78;
  const heightPx = height * 3.78;
  
  // Ölçekleme hesapla - preview alanına göre dinamik
  ...
  if (droppedElements.length === 0) {
    container.innerHTML = `
      <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); 
        ...
        Elementleri sürükleyip bırakın
      </div>
    `;
  } else {
...
function getElementSampleData(type) {
  const sampleData = {
    ...
    package_name: document.getElementById('samplePackageName')?.value || 'Örnek Paket',
    package_barcode: document.getElementById('samplePackageBarcode')?.value || '1234567890123',
    main_product: document.getElementById('sampleMainProduct')?.value || 'Örnek Ana Ürün',
...
document.addEventListener('DOMContentLoaded', function() {
  setTimeout(() => {
    ...
    // Template boyutu değiştiğinde önizlemeyi güncelle
    ...
    // Pencere boyutu değiştiğinde önizlemeyi yeniden hesapla
...
function addUploadedImage(fileName, imageData) {
  ...
  if (imageElement) {
    ...
    imageElement.textContent = `🖼️ ${fileName.substring(0, 10)}${fileName.length > 10 ? '...' : ''}`;
  }
...
// Paket verilerini gerçek API'den çekme fonksiyonu
function getPackageDataForProduct(productId, packageId) {
  // Bu fonksiyon seçili ürünün paket bilgilerini API'den çeker
  return fetch(`/api/products/${productId}/packages/${packageId}`)
    ...
    .then(packageData => {
      return {
        ...
        main_product: packageData.product_name || 'Ana Ürün',
...
// Global değişkenler ve script başlatıcı
let currentFilteredProducts = null; // Global değişken

// DEBUG
console.log('📝 Script yüklendi, değişkenler kontrol ediliyor...');
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 DOM hazır, ürünler yükleniyor...');
    refresh();
  });
} else {
  console.log('✅ DOM zaten hazır, hemen yükle');
  setTimeout(() => refresh(), 100); // Kısa bir gecikme ile
}
</script>
